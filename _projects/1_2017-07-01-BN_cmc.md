...
**Batch normalization** (BN) is regarded as the necessary component in many well-known neural network architectures to speed up network training phrase and also to improve performance. In this work, we demonstrate the effectiveness of BN by comprehensive experiments and also investigate the underlying reasons, in the theoretical point of view, that why batch normalization works so well.
<br /> We have demonstrate the following effects:
...
- BN does **improve the gradient flows** through the network (verified by examining the isometry of backward propagation)
